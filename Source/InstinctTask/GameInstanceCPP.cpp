...
bool UGameInstanceCPP::MyTick(float DeltaSeconds)
{
	//an alternative method to a delay node in Blueprints. 
...
void UGameInstanceCPP::SpawnCoins()
{
	...
	for (int i = 0; i <= TotalNumberOfCoins; i++)
	{
		...
		MyActorSpawnParameters.SpawnCollisionHandlingOverride = ESpawnActorCollisionHandlingMethod::AdjustIfPossibleButAlwaysSpawn;
...
void UGameInstanceCPP::SpawnEnemyTurrets()
{
...
void UGameInstanceCPP::SetVariablesFromFile()
{
...
bool UGameInstanceCPP::SpawnPlaneAtLocation(FVector Location, FRotator Rotation, FVector Scale)
{
